...
def run_cycle_wrapper():
    ...
    try:
        logger.info(f"[META] Summary keys: {list(summary.keys())}")
        if "eval_metrics" in summary:
            logger.info(f"[META] Eval metrics: {summary['eval_metrics']}")
        if "client_exports" in summary:
            logger.info(f"[META] Client exports: {summary['client_exports']}")
    except Exception as e:
        logger.warning(f"[META] Gagal log summary: {e}")
